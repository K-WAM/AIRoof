...
| 3 Boundary applications | T-030…T-035 | 30% | **in progress** (T-030 assigned; T-033/T-035 assigned in parallel, no file overlap) | Phase 1+2 merged; see agentTools.ts serialization in MASTER_PLAN §Integration order |
...
Overall implementation: **35%** (Phases 0+1+2 merged and combined-gate verified on main at `b16493e`, 2026-07-20:
type-check clean, lint 0 errors/26 baseline warnings, `npm test` 106/107 — 1 pre-existing flake in
`src/test-utils/example-lib.test.ts` that passes in isolation, fails only under full-suite parallel load;
unrelated to any Phase 2 owned file — `npm run build` not re-run this cycle, no source files changed by doc-only
integrator commit).
...
| Worker C — Codex Sol 5.6 (extra high) | `D:\Apps\air-wt-scheduling-integrity` | `task/scheduling-integrity` | T-030 | `checkAvailability`/`bookAppointment` in `src/lib/tools/agentTools.ts`; `src/app/api/jobs/[jobId]/assign/route.ts`; `src/app/api/appointments/[appointmentId]/route.ts`; `src/app/company/calendar/page.tsx` | **assigned** — worktree created, no commits yet |
...
**Assignment rationale (C/D, Phase 3):** T-030 (transactional booking/calendar conflict checks, optimistic-UI rollback, cross-cutting `agentTools.ts` change) needs the same adversarial rigor as Batch A/A2 — routed to Codex, single owner per MASTER_PLAN (`agentTools.ts` merge order forbids concurrent edits: T-011 → T-030 → T-031 → ...). T-035 (demo/prod isolation: allowlist constant, marker check, pre-delete backup, reset lock, explicit confirm field) is contained to 3 files, no crypto/token design, same "fail-closed guard-rail" shape as T-020 — routed to Deepseek. Verified zero file overlap between T-030's and T-035's owned scopes, and both tasks' Deps (T-011/T-021/T-022 for T-030; T-020 for T-035) are already merged, so both can develop in parallel even though MASTER_PLAN's Integration Order lists T-035 as merging *after* T-030/T-031 — that ordering constrains merge sequencing, not development start. T-031/T-032/T-034 stay queued: T-031 must serialize after T-030 on `agentTools.ts`; T-032 touches `createLead` in the same file (same constraint); T-034 modifies the protected `verifyRole.ts` guard and needs new token/crypto design (HMAC, TTL, revocation) — that rigor profile matches Codex, not a second parallel Deepseek task, so it waits for either Codex's next free slot or explicit owner direction to open a third worktree.
