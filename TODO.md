...
- [ ] Phase 25 — Demo feedback round 2 (owner-directed, 2026-09-25/26). Spec + prompts: `docs/DEMO-FEEDBACK-PLAN.md`; queue: `docs/WORKER_QUEUE.md` section **E**.
  ...
  - [x] **E6a** (2026-09-26, merged; Codex) — explicit `pairId` on After photos ("Pairs with…" in the edit sheet), sortable Photos tab (mouse/touch/keyboard), `PATCH /photos/order` (office-only, tenant-scoped), pure `photoPages()`. **Not verified in a real browser** (worker's browser tool failed; drag-and-drop covered by unit/route tests only — check on the demo). **E6b** (photos on quote/invoice/report, server-side loading for email) is next and unblocked.
...
| NH-21 | **Twilio number** — imported 2026-09-24: +1 689 204 2643 (Kissimmee FL) is attached to the ElevenLabs agent (`phnum_0801m3aknbref2w9tbhqc6ad3xzb`). STILL TODO (owner): click **Upgrade** on the Twilio account before any prospect demo (trial plays a trial message); optionally buy a **561 (Boca Raton)** number after upgrading; rotate the Twilio Auth Token once everything works. | Prospect demos on the ElevenLabs line | Full steps: `docs/NEEDS-HUMAN-CHECKLIST.md` |
