...
export function PhotoEditSheet({
  ...
  canCurate?: boolean;
  onClose: () => void;
  onSaved: (patch: Partial<JobPhotoMeta>) => void;
  onDeleted?: () => void;
}) {
  ...
  async function save() {
    ...
    setError(null);
    try {
      const patch: Record<string, unknown> = { businessId, label: label.trim(), phase };
      ...
      onSaved({ label: label.trim(), phase, ...(canCurate ? { includeInReport } : {}) });
      onClose();
    } catch {
