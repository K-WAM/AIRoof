...
function FieldApp() {
  ...
  async function submit() {
    if (!selectedJobId || !text.trim()) { setError("Select a job and record an update."); return; }
    setError(null);
    setSubmitting(true);
    ...
    try {
      const res = await fetch(`/api/jobs/${selectedJobId}/updates`, {
        ...
        body: JSON.stringify({
          ...
          jobContext: { title: selectedJob?.title, address: selectedJob?.address, serviceType: selectedJob?.serviceType, clientName: selectedJob?.clientName },
        }),
      });
      if (res.ok) {
        setSubmitted(true);
        ...
        countdownRef.current = setInterval(() => {
          setCountdown(c => { if (c <= 1) { clearInterval(countdownRef.current!); setSubmitted(false); setError(null); return 5; } return c - 1; });
        }, 1000);
  ...
  function reset() { clearInterval(countdownRef.current!); setSubmitted(false); setError(null); setCountdown(5); }
    ...
    return (
      <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#f0fdf4", padding: 24 }}>
        <div style={{ textAlign: "center", maxWidth: 360, width: "100%" }}>
          ...
          <button onClick={reset} style={{ width: "100%", padding: "16px", background: "#16a34a", color: "#fff", border: "none", borderRadius: 14, fontSize: 17, fontWeight: 700, cursor: "pointer", marginBottom: 10 }}>Submit another</button>
  ...
  return (
    <>
      ...
      <div style={{ padding: "20px 16px 120px", maxWidth: 480, margin: "0 auto" }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ position: "relative" }}>
            <select value={selectedJobId} onChange={e => setSelectedJobId(e.target.value)} disabled={loadingJobs} style={{ width: "100%", padding: "14px 44px 14px 16px", borderRadius: 12, border: "1.5px solid #e2e8f0", fontSize: 16, fontWeight: 500, color: selectedJobId ? "#0f172a" : "#94a3b8", background: "#fff", appearance: "none", WebkitAppearance: "none", cursor: "pointer", outline: "none" }}>
        ...
        {voiceSupported && (
          <div style={{ textAlign: "center", margin: "28px 0 24px" }}>
            <div style={{ position: "relative", display: "inline-block", marginBottom: 14 }}>
              ...
              <button onPointerDown={handlePressStart} onPointerUp={handlePressEnd} onPointerLeave={handlePressEnd} onPointerCancel={handlePressEnd} style={{ width: 128, height: 128, borderRadius: "50%", border: "none", background: pressing ? "linear-gradient(135deg, #dc2626, #ef4444)" : "linear-gradient(135deg, #1d4ed8, #3b82f6)", cursor: "pointer", fontSize: 52, position: "relative", zIndex: 1, boxShadow: pressing ? "0 8px 32px rgba(239,68,68,0.5)" : "0 8px 32px rgba(59,130,246,0.4)", transform: pressing ? "scale(0.94)" : "scale(1)", transition: "background 0.15s, box-shadow 0.2s, transform 0.12s", userSelect: "none", WebkitUserSelect: "none", touchAction: "none" }}>🎤</button>
            </div>
            <p style={{ margin: 0, fontWeight: 700, fontSize: 14, color: pressing ? "#ef4444" : "#64748b", transition: "color 0.15s" }}>{pressing ? "Recording — release to pause" : "Hold to record"}</p>
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#94a3b8" }}>Hold mic · release · hold again · tap Submit when done</p>
          </div>
        )}

        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em", display: "block", marginBottom: 8 }}>Update {voiceSupported ? "— or type" : ""}</label>
          <textarea value={text} onChange={e => setText(e.target.value)} placeholder={voiceSupported ? "Hold the mic above and speak, or type here…" : "Type your update here…"} rows={5} style={{ width: "100%", padding: "14px 16px", borderRadius: 12, border: pressing ? "2px solid #ef4444" : "1.5px solid #e2e8f0", fontSize: 15, lineHeight: 1.65, resize: "vertical", color: "#0f172a", outline: "none", transition: "border-color 0.15s", background: "#fff" }} />
          {interimText && (
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input value={workerName} onChange={e => setWorkerName(e.target.value)} placeholder="e.g. Miguel" style={{ width: "100%", padding: "13px 16px", borderRadius: 12, border: "1.5px solid #e2e8f0", fontSize: 15, color: "#0f172a", outline: "none", background: "#fff" }} />
        </div>

        {error && <div style={{ padding: "12px 16px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, color: "#b91c1c", fontSize: 14, fontWeight: 600, marginBottom: 16 }}>{error}</div>}
      </div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#fff", borderTop: "1px solid #f1f5f9", padding: "12px 16px", paddingBottom: "calc(12px + env(safe-area-inset-bottom))" }}>
        <button onClick={submit} disabled={!canSubmit} style={{ width: "100%", padding: "16px", borderRadius: 14, border: "none", background: canSubmit ? "linear-gradient(135deg, #1d4ed8, #3b82f6)" : "#e2e8f0", color: canSubmit ? "#fff" : "#94a3b8", fontWeight: 800, fontSize: 17, cursor: canSubmit ? "pointer" : "not-allowed", transition: "background 0.15s", boxShadow: canSubmit ? "0 4px 16px rgba(59,130,246,0.35)" : "none" }}>
