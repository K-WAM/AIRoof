...
import { quoteTotal, visibleQuoteLines } from "@/lib/billing/jobQuote";

export function QuotePanel({ job, businessId, onStatus }: { job: Job; businessId: string; onStatus: (status: Job["status"]) => void }) {
  ...
  async function save() {
    ...
    try {
      const r = await fetch(`/api/jobs/${job.jobId}/quote`, { method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessId, lines: quote.lines, findings: quote.findings, notes: quote.notes ?? "", hideMaterials: quote.hideMaterials, validUntil: quote.validUntil }) });
  ...
  return <section className="panel"><div className="panel-header"><h2 className="panel-title">Quote {quote?.quoteId ?? ""}</h2></div><div className="panel-body" style={{ display: "grid", gap: 16 }}>
    ...
    {!quote ? <button className="button primary" disabled={busy} onClick={create}>Create draft quote</button> : <>
      ...
      <label><input type="checkbox" disabled={quote.status !== "draft"} checked={quote.hideMaterials} onChange={(e) => change({ hideMaterials: e.target.checked })}/> Hide materials breakdown from customer</label>
      <div style={{ border: "1px solid #e2e8f0", padding: 12, borderRadius: 8, overflowWrap: "anywhere" }}><strong>Customer view</strong>{visibleQuoteLines(quote).map((line) => <div key={line.lineId}>{line.description} · {line.quantity} × ${line.unitPrice.toFixed(2)}</div>)}<strong>Estimated total: ${quoteTotal(quote.lines).toFixed(2)}</strong></div>
