...
describe("offline demo-path smoke test", () => {
  ...
  it("walks the demo story through real route handlers", async () => {
    ...
    expect(db.__peek(`businesses/${BUSINESS_ID}/jobs`, job.jobId)?.status).not.toBe("invoiced"); // a draft bills nobody
    ...
    expect(db.__peek(`businesses/${BUSINESS_ID}/jobs`, job.jobId)).toMatchObject({ status: "invoiced", statusHistory: expect.arrayContaining([expect.objectContaining({ status: "invoiced" })]) });
    expect((await invoicePatch(json(`http://localhost/api/jobs/${job.jobId}/invoice`, { businessId: BUSINESS_ID, status: "paid" }, "PATCH"), context(job.jobId))).status).toBe(200);
    expect((await invoicePatch(json(`http://localhost/api/jobs/${job.jobId}/invoice`, { businessId: BUSINESS_ID, status: "paid" }, "PATCH"), context(job.jobId))).status).toBe(409);
