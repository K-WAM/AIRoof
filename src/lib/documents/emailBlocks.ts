...
export function totalBlock(label: string, amount: number): string {
  return `<div style="border:2px solid #0f766e;border-radius:6px;padding:12px 16px;text-align:right;font-size:17px;font-weight:800">${escapeHtml(label)}: ${money(amount)}</div>`;
}
