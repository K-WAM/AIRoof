...
import { billToBlock, documentShell, footerBlock, groupsBlock, letterheadBlock, narrativeBlock, notesBlock, totalBlock } from "@/lib/documents/emailBlocks";
import { resolveLetterhead, type LetterheadBusiness } from "@/lib/documents/letterhead";

export type InvoiceEmailBusiness = LetterheadBusiness;
...
export function buildJobInvoiceEmailHtml(invoice: JobInvoice, business: InvoiceEmailBusiness): string {
  ...
  return documentShell(letterheadBlock(brand, "Invoice", meta) + billToBlock(invoice.billTo) + narrativeBlock(invoice.narrative) + groupsBlock(invoiceGroups(invoice)) + totalBlock("Total Due", invoice.total, brand.brandColor) + notesBlock(invoice.notes) + footerBlock(brand));
}
