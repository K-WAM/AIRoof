...
export function buildJobReportEmailHtml(input: {
  ...
  const meta: [string, string][] = [["Date", fmtDate(Date.now(), input.business.timezone ?? "America/New_York")], ["Reference", input.jobId]];
  ...
  return documentShell(letterheadBlock(brand, "Report", meta) + billToBlock(input.billTo, "Prepared for") + narrativeBlock(stripHiddenFacts(input.narrative ?? "", input.options)) + findingsBlock(selectedFindings) + sectionsBlock(sections) + pairedPhotos + otherPhotos + footerBlock(brand));
}
