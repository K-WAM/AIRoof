...
import { billToBlock, documentShell, findingsBlock, footerBlock, groupsBlock, letterheadBlock, narrativeBlock, totalBlock } from "@/lib/documents/emailBlocks";
import { pairReportPhotos, reportGroups, reportTotal, type ReportPhoto } from "@/lib/documents/report";
...
export function buildJobReportEmailHtml(input: {
  ...
  const groups = reportGroups(input.parsed, input.defaultRate, input.options);
  ...
  return documentShell(letterheadBlock(brand, "Report", meta) + billToBlock(input.billTo) + narrativeBlock(input.narrative) + findingsBlock(selectedFindings) + groupsBlock(groups) + totalBlock("Estimated total", reportTotal(groups), brand.brandColor) + pairedPhotos + otherPhotos + footerBlock(brand));
}
